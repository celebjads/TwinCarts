 ...
 * --------------------------------------------------------------------------- */
function coolstuff_fnc_woocommerce_query($type,$post_per_page=-1,$cat=''){
...
function coolstuff_fnc_get_review_counting(){
...
function coolstuff_fnc_price( $price ){
    return $price;
}


function coolstuff_fnc_woocommerce_getcategorychilds( $parent_id, $pos, $array, $level, &$dropdown ) {
    ...
    for ( $i = $pos; $i < count( $array ); $i ++ ) {
        if ( isset($array[ $i ]) && $array[ $i ]->category_parent == $parent_id ) {
            ...
            coolstuff_fnc_woocommerce_getcategorychilds( $array[ $i ]->term_id, $i, $array, $level + 1, $dropdown );
        }
    }
}
            ...
            <?php if ( have_posts() ) : ?>
                ...
                <?php woocommerce_product_loop_start(); ?>
                    ...
                    <?php while ( have_posts() ) : the_post(); ?>
                        ...
                        <?php wc_get_template_part( 'content', 'product-shop' ); ?>
...
function coolstuff_woocommerce_fnc_before_shop_loop_item_title(){
    global $product;
