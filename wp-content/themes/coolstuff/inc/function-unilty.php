...
if( !function_exists("coolstuff_fnc_wpml_language_buttons") ){
   function coolstuff_fnc_wpml_language_buttons(){
  ...
  */
function coolstuff_fnc_get_header_layouts(){
    $path = get_template_directory().'/header-*.php';
    $files = glob( $path  );
    $headers = array( '' => esc_html__('Default', 'coolstuff') );
    if( count($files)>0 ){
        foreach ($files as $key => $file) {
            $header = str_replace( "header-", '', str_replace( '.php', '', basename($file) ) );
            $headers[$header] = esc_html__( 'Header', 'coolstuff' ) . ' ' .str_replace( '-',' ', ucfirst( $header ) );
        }
    }
  ...
  */
function coolstuff_fnc_get_footer_profiles(){
    
    $footers_type = get_posts( array('posts_per_page' => -1, 'post_type' => 'footer') );
    $footers = array(  '' => esc_html__('None', 'coolstuff') );
 ...
 */
function coolstuff_fnc_get_menugroups(){
 ...
 *
 */
function coolstuff_fnc_cst_skins(){
 ...
 * 
 */
function coolstuff_fnc_render_post_content( $footer ){

  global $coolstuff_wpopconfig; 

	$post = get_post( $footer );
  $coolstuff_wpopconfig['type'] = 'footer';
	if($post){
    	echo do_shortcode( $post->post_content ); 
  }
  
  $coolstuff_wpopconfig['type'] = '';
 ...
 */
if(!function_exists('coolstuff_fnc_makeid')){
    function coolstuff_fnc_makeid($length = 5){
...
if(!function_exists('coolstuff_fnc_excerpt')){
    //Custom Excerpt Function
    function coolstuff_fnc_excerpt($limit,$afterlimit='[...]') {
...
function coolstuff_fnc_get_widget_block_styles(){
