...
<?php
	...
	function process_attachment( $post, $url ) {
		...
		$post_id = $this->pn_get_attachment_id_from_url( $upload['url'] ) ;
		if(  !$post_id ){
	...
	function fetch_remote_file( $url, $post ) {
		...
		if(  preg_match("#.png#", $file_name) ||  preg_match("#.jpg#", $file_name) || preg_match("#.jpeg#", $file_name)  ){
			...
			$data = apply_filters( 'pbrthemer_import_attachment_image_size', $url  );
			

			if( !empty($data) && isset($data['allowed']) && $data['allowed'] ){
				
				$image_folder = '2016/05/';
				$wp_upload_dir        = wp_upload_dir();
				$dir_up  = $wp_upload_dir['basedir'] . '/' . $image_folder;
				if( !is_dir($dir_up) ){
					PbrImageImportHelper::create_dir( $wp_upload_dir['basedir'],  $image_folder  );
				}
				...
				if( !is_file($upp) ){
					PbrImageImportHelper::create_image( $data['width'], $data['height'], 'CCCCCC', 'DDDDDD',  $upp );
				}	
