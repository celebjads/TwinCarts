...
class PbrThemer_Import {
	...
	public function __construct() {
		...
		if( is_dir( PBR_THEMER_IMPORT_FOLDER ) ){
	        ...
	        $this->theme = apply_filters( 'pbrthemer_import_theme', 'pbrthemer' );
	...
	public function import_customizer_options($file){
		...
		$this->message = __("Customizer options imported successfully", "pbrthemer");
	}
	...
	public function import_content($file){
		...
		if (!class_exists('WP_Importer')) {
			...
			if(is_wp_error($returned_value)){
				$this->message = __("An Error Occurred During Import", "pbrthemer");
			}
			else {
				$this->message = __("Content imported successfully", "pbrthemer");
			}
			ob_get_clean();
		} else {
			$this->message = __("Error loading files", "pbrthemer");
		}
		return ;
	}
	...
	public function import_widgets( $file ){
		...
		$this->message = __("Widgets imported successfully", "pbrthemer");
	}
	...
	public function get_file_content_unserialize( $file ){
		...
		if ( file_exists($file_for_import) ) {
			...
			$this->message = __("File doesn't exist", "pbrthemer");
		}
		if ($file_content) {
	...
	public function admin_import_page() {
		...
		$theme = wp_get_theme();
		$this->pagehook = add_submenu_page( 'themes.php',  __( 'WpOpal Import' ), esc_html__( 'WpOpal Import', 'pbrthemer'), 'manage_options', 'pbrthemer_options_import_page', array(&$this, 'render_admin_import_page'),'dashicons-download');
	...
	public function render_admin_import_page() {
		...
		?>
        <div class="wrap">
            <h2 class="pbrthemerf-page-title"><?php _e( $theme->Name . ' - One-Click Import', 'pbrthemer') ?></h2>
            ...
            <form method="post" action="" id="importContentForm">
                <table class="form-table">
                	...
                	<tr>
                		<td colspan="2">
                			...
                			<div class="update-nag">
				                <?php _e( "Click to the follow buttons to get sample demo from our live sites, the package will put into THEME/inc/import folder.<br> Or you can copy the demo package from the downloadable you purchased. <br> Please make sure this folder has writeable permision",'pbrthemer');?>
							</div> 
							<?php 
							if( !is_writable(PBR_THEMER_IMPORT_FOLDER) ){ ?><br>
							<div class="update-nag clearfix clear">
				                <?php _e( "THEME/inc/import folder is not writeable",'pbrthemer');?>
							</div> 
							<?php }
							?>
                		</td>	
                	 ...
                	 <tr>
                        <th class="row"><?php _e( 'Get Demo From Live Server', 'pbrthemer') ?></th>
                        <td>
                           <ul> 
                   ...
                   <tr>
                    	<td colspan="2">
	            		 	 <div class="update-nag">
				                <?php _e( "Please wait when the import is running! It will take time needed to download all attachments from demo web site.",'pbrthemer');?>
							</div>

                    	</td>
                    </tr>	
                    <tr>
                        <th class="row"><?php _e( 'Demo Source', 'pbrthemer') ?></th>
                        <td>
                    ...
                    <tr>
                        <th class="row"><?php _e( 'Import Type', 'pbrthemer') ?></th>
                        <td>
                            <select name="import_type" id="import_type">
                                <option value=""><?php _e( "Please Select ",'pbrthemer'); ?></option>
                    ...
                    <tr>
                    	...
                    	<td>
                    	    <p class="submit">
                    	        <input type="submit" name="submit" id="import" class="button button-primary" value="<?php _e( 'Import', 'pbrthemer') ?>">
					...
					<tr>
                    	<td colspan="2">
	            		 	 <div class="update-nag">
				                <?php _e( "This theme has not any package ready for installing demo.",'pbrthemer');?>
							</div>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                ...
                $importBtn.on('click', function(e) {
                    ...
                    if ( confirm('<?php _e("Do you want to import demo now?", "pbrthemer"); ?>') ) {
